...
class AlignedProcessGenerator:
    ...
    def generate(self, n_samples=10000, save_path="data/synthetic_aligned/dataset.csv"):
        ...
        for _ in range(n_samples):
            ...
            # Table 5: φ = 0.35-0.45, 扩展到 0.32-0.48 以完全覆盖并增加样本量
            phi_2 = np.random.uniform(0.32, 0.48)
            ...
            # G_max 调整 - 进一步提高基准值以适应更低的 Phi 范围
            g_max_base = 250000.0  # 提高到 250000
            g_max_1 = g_max_base * (1 + 0.08 * (t_temp_1 - 25.0))
            m1_1 = calc_m1(t_d50, g_max_1)

            # Phi_m0 调整 - 进一步降低,使得低 Phi 值也能产生合理的 Tau0
            phi_m0 = 0.56 + 0.06 * (t_sigma - 1.2)  # 调整到 0.56
        ...
        # 过滤条件: 严格对齐 Table 5 范围
        # - Tau0_1 在 [10, 60] Pa (对齐 Table 5: 10-50 Pa)
        # - Tau0_2 在 [10, 60] Pa
        # - Phi < Phi_m (物理约束)
        df_filtered = df[
            (df['Tau0_1(屈服应力_Pa)'] >= 10.0) & (df['Tau0_1(屈服应力_Pa)'] <= 60.0) &
            (df['Tau0_2(屈服应力_Pa)'] >= 10.0) & (df['Tau0_2(屈服应力_Pa)'] <= 60.0) &
...
if __name__ == "__main__":
    ...
    gen.generate(n_samples=100000, save_path="data/synthetic_aligned/dataset.csv")
